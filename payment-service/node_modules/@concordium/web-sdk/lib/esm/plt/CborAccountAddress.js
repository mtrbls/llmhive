var _CborAccountAddress_nominal;
...
class CborAccountAddress {
    ...
    coinInfo) {
        ...
        _CborAccountAddress_nominal.set(this, true);
    }
    toString() {
...
_CborAccountAddress_nominal = new WeakMap();
/**
...
function fromCBORValueAccount(decoded) {
    ...
    if (coinInfo !== undefined) {
        ...
        if (!(coinInfo instanceof Tag) || coinInfo.tag !== TAGGED_COININFO) {
            throw new Error(`Invalid CBOR encoded token holder account: coin info has incorrect tag (expected ${TAGGED_COININFO})`);
        }
        ...
        if (!(coinInfoMap instanceof Map) || coinInfoMap.get(1) !== CCD_NETWORK_ID) {
            throw new Error(`Invalid CBOR token holder account: coin info does not contain Concordium network identifier ${CCD_NETWORK_ID}`);
        }
...
//# sourceMappingURL=CborAccountAddress.js.map