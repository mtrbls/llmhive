...
import { CborAccountAddress, TokenAmount, TokenMetadataUrl, decodeTokenOperations, } from './index.js';
function decodeTokenModuleState(value) {
    ...
    try {
        if ('metadata' in decoded)
            metadata = TokenMetadataUrl.fromCBORValue(decoded.metadata);
    }
    catch {
...
function decodeTokenModuleAccountState(value) {
    const decoded = cborDecode(value.bytes);
    if (typeof decoded !== 'object' || decoded === null) {
...
function decodeTokenInitializationParameters(value) {
    const decoded = cborDecode(value.bytes);
    if (typeof decoded !== 'object' || decoded === null) {
 ...
 */
export function decode(cbor, type) {
    switch (type) {
