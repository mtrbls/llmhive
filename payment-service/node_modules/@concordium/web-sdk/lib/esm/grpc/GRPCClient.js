...
import { isRpcError } from '../types.js';
...
export class ConcordiumGRPCClient {
    ...
    async sendRawAccountTransaction(header, energyAmount, payload, signature) {
        ...
        if (TransactionExpiry.toDate(header.expiry) < new Date()) {
            throw new Error('A transaction expiry is not allowed to be in the past: ' + TransactionExpiry.toDate(header.expiry));
        }
    ...
    getAccountNonFinalizedTransactions(accountAddress, abortSignal) {
        const transactions = this.client.getAccountNonFinalizedTransactions({ value: AccountAddress.toBuffer(accountAddress) }, { abort: abortSignal }).responses;
    ...
    async findInstanceCreation(address, from, to) {
        return this.findEarliestFinalized(async ({ hash, height }) => {
            try {
    ...
    async findFirstFinalizedBlockNoLaterThan(time, from, to) {
        return this.findEarliestFinalized(async ({ hash }) => {
     ...
     */
    async getTokenInfo(tokenId, blockHash) {
        const blockHashInput = getBlockHashInput(blockHash);
        const req = {
     ...
     */
    getTokenList(blockHash, abortSignal) {
        const blockHashInput = getBlockHashInput(blockHash);
 ...
 * @hidden
 */
export function getBlockHashInput(blockHash) {
    if (blockHash) {
        return {
            blockHashInput: {
                oneofKind: 'given',
 ...
 * @hidden
 */
export function getAccountIdentifierInput(accountIdentifier) {
 ...
 * @hidden
 */
export function getInvokerInput(invoker) {
    if (!invoker) {
        return undefined;
    }
