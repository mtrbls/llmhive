...
import { AttributeKeyString, AttributesKeys } from '../types.js';
import { compareStringAttributes, isStringAttributeInRange, statementAttributeTypeToAttributeType, timestampToDate, } from './helpers.js';
import { IDENTITY_SUBJECT_SCHEMA, isTimestampAttribute, } from './types.js';
 ...
 */
export function verifyAtomicStatements(statements, schema) {
...
export class AtomicStatementBuilder {
    constructor(schema) {
        this.statements = [];
        this.schema = schema;
    }
...
export class Web3StatementBuilder {
    ...
    addForIdentityCredentials(validIdentityProviders, builderCallback) {
        return this.add(getAccountCredentialQualifier(validIdentityProviders), builderCallback, IDENTITY_SUBJECT_SCHEMA);
    }
    getStatements() {
        return this.statements;
    }
 ...
 */
export function createWeb3IdDID(network, publicKey, index, subindex) {
    return ('did:ccd:' +
        network.toLowerCase() +
        ':sci:' +
        index.toString() +
 ...
 * Create a DID string for a web3id credential. Used to build a request for a verifiable credential.
 */
export function createAccountDID(network, credId) {
 ...
 */
export function createAccountCommitmentInput(statements, identityProvider, attributes, randomness) {
    return {
        type: 'account',
 ...
 */
export function createAccountCommitmentInputWithHdWallet(statements, identityProvider, attributes, wallet, identityIndex, credIndex) {
    const randomness = statements.reduce((acc, x) => {
        acc[x.attributeTag] = wallet
            .getAttributeCommitmentRandomness(identityProvider, identityIndex, credIndex, AttributesKeys[x.attributeTag])
            .toString('hex');
        return acc;
    }, {});
 ...
 */
export function createWeb3CommitmentInput(verifiableCredentialPrivateKey, credentialSubject, randomness, signature) {
    return {
        type: 'web3Issuer',
 ...
 */
export function createWeb3CommitmentInputWithHdWallet(wallet, issuer, credentialIndex, credentialSubject, randomness, signature) {
    return createWeb3CommitmentInput(wallet.getVerifiableCredentialSigningKey(issuer, credentialIndex).toString('hex'), credentialSubject, randomness, signature);
}
/**
...
function isInRange(value, lower, upper) {
    ...
    if (isTimestampAttribute(value) && isTimestampAttribute(lower) && isTimestampAttribute(upper)) {
        return (timestampToDate(lower).getTime() <= timestampToDate(value).getTime() &&
 ...
 */
export function canProveAtomicStatement(statement, attributes) {
 ...
 */
export function canProveCredentialStatement(credentialStatement, attributes) {
