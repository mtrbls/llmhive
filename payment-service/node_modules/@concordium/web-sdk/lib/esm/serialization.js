...
import { encodeWord8, encodeWord8FromString, encodeWord16, encodeWord32, encodeWord64, serializeMap, serializeVerifyKey, serializeYearMonth, } from './serializationHelpers.js';
import { AttributesKeys, BlockItemKind, } from './types.js';
 ...
 */
export function serializeAccountTransactionSignature(signatures) {
 ...
 */
export function serializeAccountTransaction(accountTransaction, signatures) {
    ...
    const energyCost = calculateEnergyCost(countSignatures(signatures), BigInt(serializedPayload.length), baseEnergyCost);
    const serializedHeader = serializeAccountTransactionHeader(accountTransaction.header, serializedPayload.length, energyCost);
    return Buffer.concat([
 ...
 */
export function serializeAccountTransactionPayload(accountTransaction) {
 ...
 */
export function getAccountTransactionHash(accountTransaction, signatures) {
 ...
 */
export function getAccountTransactionSignDigest(accountTransaction, signatureCount = 1n) {
    const accountTransactionHandler = getAccountTransactionHandler(accountTransaction.type);
    const serializedPayload = serializeAccountTransactionPayload(accountTransaction);
    const baseEnergyCost = accountTransactionHandler.getBaseEnergyCost(accountTransaction.payload);
 ...
 */
export function serializeAccountTransactionForSubmission(accountTransaction, signatures) {
    const serializedAccountTransaction = serializeAccountTransaction(accountTransaction, signatures);
...
function serializeIdOwnershipProofs(proofs) {
    ...
    const idCredPubProofs = Buffer.concat(Object.entries(proofs.proofIdCredPub)
 ...
 */
export function getCredentialForExistingAccountSignDigest(unsignedCredentialDeploymentInfo, address) {
 ...
 */
export function getCredentialDeploymentSignDigest(credentialDeployment) {
